...
ocean_m = plt.imread('map/map-1.png')
...
plt.show()
    ...
    # df = pd.read_csv('data/seafloor/EMODNET_MLDB_aggregated_collection_seafloor.csv')
    # df.sort_values("HaulLong", inplace = True)

    # filter1 = df["HaulLong"]==pos[0]
    # filter2 = df["HaulLat"]==pos[1]

    # df.where(filter1 & filter2, inplace = True)
    # print(df)

    # print("Null", df['HaulLong'].isnull().sum())
    # print("Not null", df['HaulLong'].notnull().sum())